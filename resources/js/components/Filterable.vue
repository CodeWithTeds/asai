...
<template>
    <div class="flex items-center gap-2">
        ...
        <DropdownMenu v-for="group in filterGroups" :key="group.key">
            <DropdownMenuTrigger as-child>
                <button
                    class="inline-flex h-9 items-center gap-1.5 rounded-md border border-input bg-transparent px-3 text-sm text-muted-foreground shadow-xs transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none"
                    :class="
                        hasActiveFilter(group.key)
            ...
            <DropdownMenuContent align="start" class="w-44">
                <DropdownMenuRadioGroup
                    :model-value="localFilters[group.key] ?? ''"
                >
                    <DropdownMenuRadioItem
        ...
        <button
            ...
            class="inline-flex h-9 items-center gap-1.5 rounded-md border border-input px-3 text-sm text-muted-foreground shadow-xs transition-colors hover:bg-destructive/10 hover:text-destructive focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none"
        ...
        <div class="ml-auto flex items-center gap-2">
            ...
            <div class="relative w-56">
                ...
                <input
                    ...
                    class="h-9 w-full rounded-md border border-input bg-transparent pr-8 pl-9 text-sm text-foreground shadow-xs transition-[color,box-shadow] placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none"
                />
                <button
                    v-if="localSearch"
                    class="absolute top-1/2 right-2.5 -translate-y-1/2 rounded text-muted-foreground/70 transition-colors hover:text-foreground"
                    @click="clearSearch"
                >
                    <X class="h-3.5 w-3.5" />
                </button>
